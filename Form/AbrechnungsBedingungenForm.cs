...
namespace TaMi_Kassenclient
{
    public partial class AbrechnungBedingungenForm : Form
    {
        ...
        private void BuildUi()
        {
            ...
            lvRegeln.Columns.Add("Priorit�t",90); // neue Spalte links von Fallback
            ...
            btnNeu = MakeBtn("Neu", Accent); btnBearbeiten = MakeBtn("Bearbeiten", Color.FromArgb(0,172,193)); btnDuplizieren = MakeBtn("Duplizieren", Color.FromArgb(3,155,229)); btnLoeschen = MakeBtn("L�schen", Color.IndianRed); btnSpeichernAlle = MakeBtn("Speichern", Color.FromArgb(76,175,80));
        ...
        private void RefreshList()
        {
            ...
            foreach(var r in _regeln)
            {
                ...
                item.SubItems.Add(r.Priority.ToString()); // Priorit�t
                ...
                if(!string.IsNullOrWhiteSpace(r.ResultBuchungstext)) parts.Add($"Text=\"{r.ResultBuchungstext}\"");
        ...
        private void AdjustListColumns()
        {
            try
            {
                if(lvRegeln.Columns.Count==0) return;
                int w = lvRegeln.ClientSize.Width - SystemInformation.VerticalScrollBarWidth; if (w<=0) return;
                if(lvRegeln.Columns.Count>=5)
                {
                    int nameW=Math.Max(180,(int)(w*0.22));
                    int prioW=Math.Max(70,(int)(w*0.08));
                    int fallbackW=Math.Max(80,(int)(w*0.10));
                    int ergW=Math.Max(180,(int)(w*0.25));
                    ...
                    lvRegeln.Columns[2].Width=prioW;      // Priorit�t
        ...
        private async void DeleteSelected(){ var r=GetSelectedRule(); if(r==null) return; if(MessageBox.Show(this,$"Regel '{r.Name}' l�schen?","Best�tigen",MessageBoxButtons.YesNo,MessageBoxIcon.Question)!=DialogResult.Yes) return; try { await SoftDeleteRuleBySqlAsync(r.Id);} catch { } await LoadRulesAsync(); }
